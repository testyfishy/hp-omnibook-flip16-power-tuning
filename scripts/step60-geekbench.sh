...
STAMP=$(date +%Y%m%d-%H%M); PFX=$LOGS/geekbench-$STAMP
...
printf "tier\ttask\tseconds\tt0\tt1\tsoc_joules\twork\n" > "$PFX.tasks.tsv"; : > "$PFX.marks.tsv"; : > "$PFX.readback.txt"
pgrep -x firefox >/dev/null && echo "WARNING: Firefox running — close it for clean scores"

for mode in $MODES; do
  case $mode in
    battery6)    wait_adapter 0 || exit 1; rb=$(readback); echo "$rb" | grep -q 'epp=balance_power msr=6/6' || { echo "battery policy is not balance_power + 6/6: $rb"; exit 1; };;
    ac)          wait_adapter 1 || exit 1; [ $PERF_SET = 1 ] && { /usr/local/sbin/power-switch.sh; PERF_SET=0; sleep 3; }; rb=$(readback); echo "$rb" | grep -q 'profile=balanced msr=30/37' || { echo "AC state is not balanced + 30/37: $rb"; exit 1; };;
